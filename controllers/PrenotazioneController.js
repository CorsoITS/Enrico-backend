...
class PrenotazioneController {
    ...
    static async bakeCreationUpdateData (req,res) {
        try {
            ...
            let sedi=[];
            sedi=await Sede.lista(0,luogo,true);
            logger.debug("bakeCreationData: sedi trovate:",sedi);
            return res.render("creaprenotazione",{listaPersone: persone,listaSede: sedi, current:ep}) ;
        } catch (err) {
    ...
    static async edit (req,res) {
        try {
            ...
            logger.debug("req.body:", req.body);
            if (req.body.sede) np.setSedeId(req.body.sede_id);
            if (req.body.persona_id) np.setPersonaId(req.body.persona_id);            
